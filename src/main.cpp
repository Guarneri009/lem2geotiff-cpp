...
void process_directory_tree(const fs::path& parent_dir, float nodata_value, int workers) {
    ...
    for (const auto& dir_info : leaf_directories) {
        ...
        std::string relative_path = fs::relative(dir_info.directory_path, parent_dir).string();
        std::replace(relative_path.begin(), relative_path.end(), '/', '_');
        // 親ディレクトリ自体の場合（relative_pathが"."）はディレクトリ名を使用
        if (relative_path == ".") {
            relative_path = dir_info.directory_path.filename().string();
        }
